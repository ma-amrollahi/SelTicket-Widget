...
const OrderPage: FC = () => {
  ...
  const { show, schedule } = useAppSelector(selectShowState);

  const { selected, order } = useAppSelector(selectOrderState);
    ...
    if (response.status === 200) {
      ...
      if (res_error.status === 200) {
        ...
        setResultPayment({
          ...
          tickets_count:
            selected?.seats?.length > 0 ? selected?.seats?.length : 0,
        });
      }
    }
  };
  ...
  if (!order) {
    return resultPayment !== undefined ? (
      <PaymentCard data={resultPayment} />
    ) : (
      <div className="selticket-bg-primary selticket-px-4 selticket-py-10 selticket-flex selticket-flex-col selticket-items-stretch selticket-gap-8">
        <Card>
          ...
          <div className="selticket-w-full selticket-border-t selticket-border-tertiary-container selticket-pt-6 ">
            <div className="selticket-flex selticket-justify-between selticket-items-center selticket-mb-6">
              ...
              <div className="selticket-flex selticket-items-center selticket-gap-2 selticket-text-[10px] selticket-px-4 selticket-h-8 selticket-bg-primary selticket-text-on-secondary selticket-rounded">
                <FontAwesomeIcon icon={faTicket} />
                <span>{selected.seats.length} بلیت</span>
              </div>
            </div>
        ...
        <Card>
          ...
          <ul className="selticket-pr-3">
            {[
              ...
              >
                <span className="selticket-mr-3 selticket-pt-1.5 selticket-text-xs selticket-text-on-primary-container selticket-text-justify selticket-leading-[18px]">
                  {item}
                </span>
              </li>
            ))}
        ...
        <Card>
          {loading ? (
            <div className="selticket-text-center">
              <Loader />
            </div>
          ) : (
            <>
              <div
                className={
                  "selticket-grid selticket-grid-cols-5 selticket-gap-4 selticket-text-xs selticket-text-on-primary-container selticket-mb-5"
                }
              >
                <div className="selticket-col-span-2">بلیت های شما</div>
                <div className="selticket-col-span-1 selticket-font-semibold">{`${selected.seats.length} عدد`}</div>
                <div className="selticket-col-span-2 selticket-font-semibold selticket-text-left">{`${priceFormatToman(
                  orderDetails?.final_price
                )} تومان`}</div>
              </div>
              <div
                className={
                  "selticket-grid selticket-grid-cols-5 selticket-gap-4 selticket-text-xs selticket-text-on-primary-container selticket-mb-7"
                }
              >
                <div className="selticket-col-span-2">
                  کارمزد خریدهای آنلاین
                </div>
                <div className="selticket-col-span-1 selticket-font-semibold">
                  %4
                </div>
                <div className="selticket-col-span-2 selticket-font-semibold selticket-text-left">{`${priceFormatToman(
                  orderDetails?.customer_final_price - orderDetails?.final_price
                )} تومان`}</div>
              </div>
              <hr className="selticket-mb-7" />
              <div className="selticket-flex selticket-flex-row selticket-text-on-primary selticket-justify-between selticket-items-center selticket-text-xs selticket-font-bold selticket-mb-8">
